...
int tmulscale11(long long a, long long b, long long c, long long d, long long e, long long f)
{
    long long r1;
    long long r3;
    long long r2;
...
int getspritescore(long snum, long dapicnum)
{
    switch(dapicnum)
    {
        ...
        case AMMO: if (ps[snum].ammo_amount[PISTOL_WEAPON] < max_ammo_amount[PISTOL_WEAPON]) return(10); else return(0);
...
{
    short sc;
    computergetinput(snum, syn);

    sc = 5 - ud.player_skill;
...
void computergetinput(long snum, input *syn)
{
    ...
    if (p->dead_flag)
    {
        if (dukematch_mode != 4 && p->ftq == 13)
            syn->bits |= (1<<29);
        return;
    }
    if ((p->firstaid_amount > 0) && (p->last_extra < 100))
    {
        short sc = 8 << ud.player_skill;
        if ((TRAND&255) < sc)
            syn->bits |= (1<<16);
    }
    ...
    if ((ps[goalplayer[snum]].dead_flag == 0) && cansee(x1,y1,z1-(48<<8),damysect,x2,y2,z2-(48<<8),sprite[ps[goalplayer[snum]].i].sectnum))
    {
        short sc = 8 << ud.player_skill;
        if ((char)TRAND < sc)
            syn->bits |= (1<<2);

        if ((p->curr_weapon == HANDBOMB_WEAPON) && (!(TRAND&7)))
            syn->bits &= ~(1<<2);

        if (p->curr_weapon == TRIPBOMB_WEAPON)
            syn->bits |= (PISTOL2_WEAPON<<8);
        ...
        w2 = ps[goalplayer[snum]].curr_weapon;
        switch (w2)
        {
            case DEVISTATOR_WEAPON:
                w2 = 7;
    ...
    if (goalsect[snum] < 0)
    {
        ...
        clearbufbyte(dashow2dsector,0L,(MAXSECTORS+7)>>3);
        ...
        for(splc=0,send=1;splc<send;splc++)
        {
            ...
            for(i=headspritesect[searchsect[splc]];i>=0;i=nextspritesect[i])
                if (sprite[i].picnum == SECTOREFFECTOR && sprite[i].lotag == 7)
                {
        ...
        if (goalsprite[snum] < 0)
        {
            for(k=0;k<4;k++)
            {
                /*if (k == 3)
                    ...
                    i = (krand2()%numsectors);*/
                i = k == 3 ? p->cursectnum : krand2()%numsectors;
...
#else
#pragma GLOBAL_ASM("nonmatchings/src/dukebot/computergetinput.s")
#endif
