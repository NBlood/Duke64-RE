...
short findplayer(spritetype *s,long *d)
{
    ...
    if(ud.multimode < 2)
    {
        *d = klabs2(ps[myconnectindex].oposx-s->x) + klabs2(ps[myconnectindex].oposy-s->y) + ((klabs2(ps[myconnectindex].oposz-s->z+(28<<8)))>>4);
    ...
    {
        x = klabs2(ps[j].oposx-s->x) + klabs2(ps[j].oposy-s->y) + ((klabs2(ps[j].oposz-s->z+(28<<8)))>>4);
...
short findotherplayer(short p,long *d)
{
    ...
    {
        x = klabs2(ps[j].oposx-ps[p].posx) + klabs2(ps[j].oposy-ps[p].posy) + (klabs2(ps[j].oposz-ps[p].posz)>>4);
    ...
    while(i >= 0)
    {
        if( SLT == 17 )
            if( SHT == sprite[s].hitag )
                if( (klabs2(sector[sn].floorz-hittype[s].temp_data[2]) > SP) ||
...
char checkhitswitch(short snum,long w,char switchtype)
{
    ...
    if((ud.multimode == 1 || ud.coop == 1 || dukematch_mode == 1) && lotag == (short) 65535)
    {
...
void checkhitwall(short spr,short dawallnum,long x,long y,long z,short atwith)
{
   ...
   if( ( (wal->cstat&16) || wal->overpicnum == BIGFORCE ) && wal->nextsector >= 0 )
    ...
    {
            case TILE3627:
                breakwall(TILE3727,spr,dawallnum);
                return;
...
void checkplayerhurt(struct player_struct *p,short j)
{
    ...
    {
        j &= 0xfff;
        if (j >= MAXSPRITES) gameexit("Sprite overflow caused by CHEESE\n");
...
void checkhitsprite(short i,short sn)
{
    ...
    i &= 0xfff;
    if (i >= MAXSPRITES) gameexit("Sprite overflow caused by LEMON\n");
    ...
    //            j = EGS(SECT,SX,SY,SZ-(TRAND%(48<<8)),SCRAP3+(TRAND&3),-8,48,48,TRAND&2047,(TRAND&63)+64,-(TRAND&4095)-(sprite[i].zvel>>2),i,5);
      //          sprite[j].pal = 2;
        ...
        default:
            ...
            if( sprite[sn].owner != i && sprite[i].statnum != 4)
            {
...
short getnextweapon(struct player_struct *p, short k, short j)
{
    ...
    while(1)
    {
        ...
        if (k > 10)
            k = 0;
        if (k == HANDBOMB_WEAPON && p->gotweapon[HANDBOMB_WEAPON] != 0)
        {
            if (p->ammo_amount[HANDBOMB_WEAPON] == 0)
            {
                ...
                while (i >= 0)
                {
                    if (sprite[i].picnum == HEAVYHBOMB && sprite[i].owner == p->i)
        ...
        if (p->gotweapon[k] != 0)
        {
            if (k == PISTOL_WEAPON && p->ammo_amount[PISTOL2_WEAPON] > 0)
                return PISTOL2_WEAPON;
            if (k == SHOTGUN_WEAPON && p->ammo_amount[SHOTGUN2_WEAPON] > 0)
                return SHOTGUN2_WEAPON;
            if (k == RPG_WEAPON && p->ammo_amount[RPG2_WEAPON] > 0)
                return RPG2_WEAPON;
            if (k == KNEE_WEAPON || p->ammo_amount[k] > 0)
                return k;
        }
    }
}
...
void cheatkeys(short snum)
{
    ...
    if ((sb_snum & 0xf00) == 0)
    {
        if (--p->f_376 <= 0)
        {
            if (p->curr_weapon != p->f_372)
            {
                if (p->f_372 == KNEE_WEAPON || p->f_372 == HANDREMOTE_WEAPON || (p->gotweapon[p->f_372] && p->ammo_amount[p->f_372]))
                    p->wantweaponfire = p->f_372;
            }
            p->f_376 = 0;
            p->f_378 = max(0, p->f_378-2);
        }
    }
    else
    {
        p->f_376 = 12;
        p->f_378 = min(30, p->f_378+15);
    }

    if (p->wantweaponfire >= 0 && p->kickback_pic == 0)
        checkavailweapon(p);
    
    if (p->f_374 > 0)
        p->f_374 = max(0, p->f_374-12);
    else
        p->f_374 = min(0, p->f_374+12);

    p->interface_toggle_flag = max(0, p->interface_toggle_flag-1);
    if( !(sb_snum&((15<<8)|(1<<12)|(1<<15)|(1<<16)|(1<<22)|(1<<19)|(1<<20)|(1<<21)|(1<<24)|(1<<25)|(1<<27)|(1<<28)|(1<<30)|(1<<31))) )
        p->interface_toggle_flag = min(p->interface_toggle_flag, 5);
    else if(p->interface_toggle_flag == 0)
    {
        p->interface_toggle_flag = 12;
            ...
            if( sb_snum&(1<<20) || sb_snum&(1<<27) || p->refresh_inventory)
        {
            ...
            switch(dainv)
            {
                ...
                case 8: FTA(38,p);break;
            }
        }

        if(p->last_pissed_time <= (26*218) && p->quick_kick == 0 && sprite[p->i].xrepeat > 32 && p->access_incs == 0 && p->knee_incs == 0 )
        {
            j = ( (sb_snum&(15<<8))>>8 ) - 1;
            if (10 == j || j == 11)
            {
...
void checksectors(short snum)
{
    ...
    if (p->cursectnum == -1)
    {
        if (sprite[p->i].extra > 0 && ud.clipping == 0)
        {
            quickkill(p);
            spritesound(SQUISHED, p->i);
        }
        return;
    }
    ...
    if(p->newowner >= 0)
    {
        if( klabs2(sync[snum].svel) > 768 || klabs2(sync[snum].fvel) > 768 )
        {
            i = -1;
            goto CLEARCAMERAS;
        }
    ...
    else if(!p->toggle_key_flag)
    {
        ...
        if(neartagsprite >= 0)
        {
            ...
            switch(sprite[neartagsprite].picnum)
            {
                ...
                case VIEWSCREEN2:
                    {
                        i = p->newowner == -1 ? headspritestat[1] : nextspritestat[p->newowner];

                        while(i >= 0)
                        {
                            if( PN == CAMERA1 && sprite[neartagsprite].hitag == SLT )
                            {
